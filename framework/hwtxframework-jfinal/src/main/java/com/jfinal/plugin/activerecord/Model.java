...
public abstract class Model<M extends Model> implements Serializable {
    ...
    public boolean hasAttr(String attr) {
        return getTable().hasColumnLabel(attr);
    }

    public int[] batchExecUpdate(String sql, Object[]... paramss) throws SQLException {
        Config config = getConfig();
        ...
        if (beanList != null && beanList.size() > 0) {
            ...
            try {
                ...
                for (M bean : beanList) {
                    ...
                    config.dialect.forModelSave(table, bean.getAttrs(), sb, paras);
                    if (sql == null) {
                        sql = sb.toString();
                    }
    ...
    public Page1<M> paginateOverOffset(int offset, int number, String select, String sqlExceptSelect, Object... paras) {
        Config config = getConfig();
        Connection conn = null;
        try {
            conn = config.getConnection();
            return paginateOverOffset(config, conn, offset, number, select, sqlExceptSelect, paras);
        } catch (Exception e) {
            throw new ActiveRecordException(e);
        } finally {
            config.close(conn);
    ...
    private Page1<M> paginateOverOffset(Config config, Connection conn, int offset, int number, String select, String sqlExceptSelect,
        ...
        if (size == 1)
            ...
            return new Page1<M>(new ArrayList<M>(0), 0); // totalRow = 0;

        StringBuilder sql = new StringBuilder();
        config.dialect.forPaginate(sql, offset, number, select, sqlExceptSelect);
        List<M> list = find(conn, sql.toString(), paras);
        return new Page1<M>(list, (int) totalRow);
    }
    ...
    public Page1<M> paginateOverOffset(int offset, int number, String select, String sqlExceptSelect) {
        return paginateOverOffset(offset, number, select, sqlExceptSelect, NULL_PARA_ARRAY);
    }
    ...
    public Page1<M> paginateOverOffsetByCache(String cacheName, Object key, int offset, int number, String select, String sqlExceptSelect, Object... paras) {
        ICache cache = getConfig().getCache();
        Page1<M> result = cache.get(cacheName, key);
        if (result == null) {
            result = paginateOverOffset(offset, number, select, sqlExceptSelect, paras);
            cache.put(cacheName, key, result);
        }
        return result;
    }
    ...
    public Page1<M> paginateOverOffsetByCache(String cacheName, Object key, int offset, int number, String select, String sqlExceptSelect) {
        return paginateOverOffsetByCache(cacheName, key, offset, number, select, sqlExceptSelect, NULL_PARA_ARRAY);
    }
