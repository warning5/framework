...
public class ModelExt<M extends ModelExt<M>> extends Model<M> {
    ...
    public boolean deleteById(Object id) {
        ...
        if (pseudoDelete()) {
            ...
            String sql = "update " + table.getName() + " set " + deleteColumnLabel + " = 1 where " + pKey[0] + " = ?";
    ...
    public List<M> children(Class<? extends Model> model) {
        ...
        String primaryKey[] = TableMapping.me().getTable(clazz).getPrimaryKey();
        try {
            return childModel.newInstance().find("select * from " + childTableName + " where " + foreignKey + "= ?", get(primaryKey[0]));
        } catch (Exception e) {
    ...
    public M parent(Class<? extends Model> model) {
        ...
        String[] primaryKey = TableMapping.me().getTable(clazz).getPrimaryKey();
        try {
            return (M) parentModel.newInstance().findFirst("select * from " + parentTableName + " where " + foreignKey + "= ?", get(primaryKey[0]));
        } catch (Exception e) {
            throw new ActiveRecordException(e.getMessage(), e);
        }
    }
